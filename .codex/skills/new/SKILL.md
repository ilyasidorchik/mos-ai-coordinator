...
6. End the user-facing reply with these two final lines (blank line between them):
   `[statistics.md](statistics.md): Это будет ваше 39-е обращение. Так держать!`
   ...
   non-breaking space (U+00A0) before `обращение`. After the period add a short
   motivational praise (vary it: «Так держать!», «Отличная работа!»,
   «Продолжайте в том же духе!» и т. п.). Put the draft link last,
