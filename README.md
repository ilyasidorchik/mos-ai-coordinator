...
`/telegram`  
Когда в `response/photos/` есть фото результата, пишем `/telegram` и перетаскиваем кейс в чат — агент отправит фото и подпись (итог, округ и адрес, координаты) в Телеграм-канал. Нужны подключённый [Upload-Post MCP](https://www.upload-post.com/mcp/) и заполненный `.env` (профиль Upload-Post и username канала; шаблон — [`.env.example`](.env.example)).
